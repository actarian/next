...
export function FilterSidebar({ filters, onChange }) {
  ...
  function onOptionChange(filter, option) {
        ...
        {filters && filters.map((filter, i) => (
          <Collapse key={filter.id} title={filter.title} font="11px">
            {filter.options && filter.options.map((option) => (
              option.title && <Checkbox className={styles.checkbox} key={option.id} value={filter.has(option)} onChange={() => onOptionChange(filter, option)} marginBottom="10px" disabled={option.count === 0}>
